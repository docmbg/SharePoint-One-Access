...
*/
    function AddGroupsToUser() {
        ...
        if ($("#my_SPGroupsAvailable").val().length>1) {
            alert("Please select only one group at a time.");
            return;
...
*/
    function RemoveGroupsFromUser() {
        ...
        if (($("#my_SPGroupsAssigned").val()).length === groups.assigned.length){
            alert("Please be aware that removing these users from all the selected groups does not remove them from the SP site. To do that please use the Mass Deletion tool.");}
        if ($("#my_SPGroupsAssigned").val().length >=1){
        ...
        for (var i = 0; i < new_users.logins.length; i++) {
            ...
            user_group_coll.done(function() {
                ...
                if (users.logins.length > 1) {
                    ...
                    alert(new_users.logins + " was/were added to the following groups: " + groups.assigned);
